...
import { Leaf, Menu, X, Globe, ChevronDown, LogIn, LogOut } from 'lucide-react'
import { useAuth } from '../contexts/AuthContext'
import AuthModal from './AuthModal'

const NAV_TEXT = {
  en: { diagnose: 'Diagnose', weather: 'Weather', history: 'History', about: 'About', signIn: 'Sign In', signOut: 'Sign Out', analyze: 'Analyze Crop', platform: 'AI Platform' },
  hi: { diagnose: 'निदान', weather: 'मौसम', history: 'इतिहास', about: 'परिचय', signIn: 'साइन इन', signOut: 'साइन आउट', analyze: 'फसल विश्लेषण', platform: 'एआई प्लेटफॉर्म' },
  mr: { diagnose: 'निदान', weather: 'हवामान', history: 'इतिहास', about: 'माहिती', signIn: 'साइन इन', signOut: 'साइन आउट', analyze: 'पीक विश्लेषण', platform: 'एआय प्लॅटफॉर्म' },
  te: { diagnose: 'నిర్ధారణ', weather: 'వాతావరణం', history: 'చరిత్ర', about: 'గురించి', signIn: 'సైన్ ఇన్', signOut: 'సైన్ అవుట్', analyze: 'పంట విశ్లేషణ', platform: 'ఎఐ ప్లాట్‌ఫార్మ్' },
}

const navItems = [
  { key: 'diagnose', path: '/diagnose' },
...
export default function Navbar({ lang, setLang }) {
  const text = NAV_TEXT[lang] || NAV_TEXT.en
        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between gap-4">
          <Link to="/" className="flex items-center gap-2.5 group flex-shrink-0">
            ...
            <div>
              ...
              <span className="text-[9px] font-mono tracking-[0.2em] uppercase block" style={{ color: '#2d5040' }}>{text.platform}</span>
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-3">
                    ...
                    {LANGUAGES.map((item) => (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-2.5 text-left text-sm transition-colors ${
                          lang === item.code ? 'bg-em-400/12 text-em-400' : 'text-ink-200 hover:bg-em-400/08'
                        }`}
                      >
                        <span>{item.flag}</span>
                        <span>{item.label}</span>
                        {lang === item.code && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-em-400" />}
                      </button>
                    ))}
                  </motion.div>
                )}
            ...
            {user ? (
              <div className="relative" ref={userRef}>
                ...
                >
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold font-display" style={{ background: 'linear-gradient(135deg, #059669, #34d399)', color: '#022c22' }}>
                    {user.avatar}
                  </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => openAuth('diagnose')} className="btn-primary text-sm px-5 py-2.5">
                  {text.analyze}
                </button>
              </div>
            )}
              ...
              {navItems.map(({ key, path }) => (
                <Link
                  ...
                  className={`px-4 py-3 rounded-xl text-sm font-500 transition-all ${
                    location.pathname === path ? 'bg-em-400/12 text-em-400' : 'text-ink-300 hover:bg-em-400/07 hover:text-em-300'
                  }`}
                >
                  {text[key]}
                </Link>
                ...
                {LANGUAGES.map((item) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-mono transition-all ${
                      lang === item.code ? 'bg-em-400/18 text-em-400 border border-em-400/30' : 'glass-sm text-ink-400'
                    }`}
                  >
              ...
              {user ? (
                <div className="mt-2 space-y-1">
                  <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl" style={{ background: 'rgba(52,211,153,0.07)' }}>
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold" style={{ background: 'linear-gradient(135deg, #059669, #34d399)', color: '#022c22' }}>
                      {user.avatar}
                    </div>
                    <div>
                      <p className="text-sm font-medium" style={{ color: '#ddeee5' }}>{user.name}</p>
